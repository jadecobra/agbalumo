...
### CI & Infrastructure
* **Post-Push Production Monitoring** [TRIGGER: git_push]: The agent is FORBIDDEN from marking a task as "done" or ending a session based solely on a successful `git push`. After pushing, the agent MUST immediately execute `./scripts/pushw.sh` (if available) or `gh run watch` to monitor the remote CI pipeline. Work is only considered complete when the remote branch is 'green'. If the remote CI fails, the agent MUST resolve the failure in the same session.
...
* **Local Server Persistence Lesson** [TRIGGER: session_done, development]: The agent is FORBIDDEN from ending a session if the local server is down. If any Go, HTML, or CSS files were modified, the agent MUST ensure the server is running in watch mode (`go run ./cmd/verify watch`) and verify that the latest changes are reflected in the UI before reporting success. Use `go run ./cmd/verify uptime` to confirm.
* **Post-Action Verification Mandate** [TRIGGER: session_done]: When implementing a new verification tool or rule (via `/learn` or `/build-feature`), the agent MUST immediately use that tool/rule to verify its own state in the current turn. Reporting a successful 'codification' while the system remains in a non-compliant state is a protocol violation.
...
### Testing
* **Test Parallelism Safety Lesson** [TRIGGER: test_authoring, env_variable]: The agent MUST NOT use `os.Setenv` or `os.Unsetenv` directly in Go tests. Use `t.Setenv()` (Go 1.17+) for setting env vars — it auto-restores on test completion AND panics if combined with `t.Parallel()`, preventing both cross-test contamination and parallel mutation. For tests that need env vars *unset*, use `os.Unsetenv` paired with `t.Cleanup()` to restore the original value. Raw `os.Setenv`/`os.Unsetenv` without cleanup is forbidden because Go does not guarantee test execution order across files in the same package, causing CI-only flaky failures.
...
* **Playwright Base URL Lesson** [TRIGGER: test_authoring]: Playwright tests MUST use relative paths (e.g., `await page.goto('/')`) that leverage the `baseURL` configured in `playwright.config.ts`. Hardcoding absolute URLs or guessing arbitrary ports in E2E tests is strictly prohibited to ensure test portability across local and CI environments.
   * **Visual Snapshot Platform Parity Lesson** [TRIGGER: test_authoring, ci_config_change]: Playwright visual regression snapshots MUST include linux-suffixed baselines (`*-linux.png`) alongside any darwin baselines before merging. The agent MUST run `go run ./cmd/verify snapshot-parity` before pushing. This tool verifies that linux snapshots exist and are NOT bitwise identical to their darwin counterparts. Copying snapshots to satisfy parity is a protocol violation; linux snapshots MUST be generated via Docker (e.g., using `mcr.microsoft.com/playwright`) to ensure they match the CI environment's rendering of fonts and layout.
   * **HTMX E2E Assertion Timing Lesson** [TRIGGER: test_authoring, htmx]: E2E tests asserting state changes driven by HTMX responses (hx-swap, hx-post) MUST use `page.waitForResponse()` targeting the specific endpoint before asserting class or DOM state. Bare `toHaveClass` or `toContainText` assertions immediately after `.click()` on HTMX-wired elements are forbidden — they create timing-dependent flakes that pass locally (0 retries) but fail on CI (2 retries exhausted). Retry masking is not a fix.
   * **Modal Snapshot Parity Lesson** [TRIGGER: test_authoring, visual_testing]: Any new or modified Modal (or dynamic z-index overlay) MUST include a Playwright visual snapshot test in `tests/e2e/visual.spec.ts` that explicitly triggers the modal state (e.g., via `page.click(listing_selector)`) to deterministically catch visual regressions like floating bounding boxes or transparent backgrounds without relying on non-deterministic LLM audits.
 
### Scraping & Data Extraction
* **Platform Attribution Exclusion Lesson** [TRIGGER: scraper_change, heuristic_change]: Heuristic scrapers (e.g., menu link extractors) MUST explicitly exclude platform-level marketing, informational, and attribution links (e.g., "Powered by ZingMyOrder", "/how-it-works", "/about", "/contact"). These links frequently contain keywords like "order" or "menu" but point to platform-wide landing pages rather than listing-specific content. Verification MUST include test cases that simulate mixed content (valid links + attribution links) to ensure correct prioritization and filtering.
* **Playwright Local Linux Gate** [TRIGGER: test_authoring, ui_change, ci_config_change]: Before pushing any change that touches Go handlers, templates, or CSS, the agent MUST run `go run ./cmd/verify ci --with-docker` to execute Playwright inside a Linux container. Passing only `go run ./cmd/verify ci` (without --with-docker) is insufficient — it skips the E2E job that production CI runs on ubuntu-latest.
