...
* **JavaScript Reliability & Security Lesson**: The agent MUST ensure all client-side JavaScript assets are syntactically validated in the CI pipeline using `go run cmd/verify/main.go js-syntax`. Direct inline scripts MUST be avoided in templates to maintain a strict `script-src 'self'` Content Security Policy, preventing the risk of XSS and execution failures in hardened production environments.
