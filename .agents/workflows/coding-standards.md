...
* **Playwright Base URL Lesson** [TRIGGER: test_authoring]: Playwright tests MUST use relative paths (e.g., `await page.goto('/')`) that leverage the `baseURL` configured in `playwright.config.ts`. Hardcoding absolute URLs or guessing arbitrary ports in E2E tests is strictly prohibited to ensure test portability across local and CI environments.
   * **Visual Snapshot Platform Parity Lesson** [TRIGGER: test_authoring, ci_config_change]: Playwright visual regression snapshots MUST include linux-suffixed baselines (`*-linux.png`) alongside any darwin baselines before merging. The agent MUST run `go run ./cmd/verify snapshot-parity` before pushing any branch that touches `tests/e2e/visual.spec.ts-snapshots/`. Committing darwin-only snapshots is a hard block — they will always fail on `ubuntu-latest` CI runners.
