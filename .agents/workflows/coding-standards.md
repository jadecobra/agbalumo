...
### UI & Frontend
* **Strict ViewModel Mandate** [TRIGGER: handler_change, template_change]: Handlers MUST NOT use untyped maps (e.g., map[string]any or echo.Map) to pass data to c.Render(). The agent MUST define a strictly typed struct (ViewModel) for the template data. This forces the Go compiler to act as our UI contract enforcer.
...
* **Root Hygiene Lesson** [TRIGGER: root_directory_clutter]: The agent MUST maintain a clean root directory. Temporary artifacts, test logs, and database files (e.g., `listings.db`, `test_output_detail.txt`) MUST be moved to `.tester/` using `verify janitor`. Executable binaries MUST be placed in `bin/`. The root directory MUST only contain core configuration and entry-point files as defined in the `verify root-hygiene` whitelist.
 
### Documentation
* **Documentation Single Writer Lesson** [TRIGGER: documentation_change]: Each piece of information MUST have exactly ONE canonical location. If referenced from a second location, use a file link (see docs/ATLAS.md), not a copy. Duplicate prose documents covering the same semantic domain are forbidden. Run go run ./cmd/verify doc-drift to detect stale references.
