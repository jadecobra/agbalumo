...
* The agent MUST verify all GitHub Action SHAs using the `verify-shas` tool. Remote verification via `gh` CLI is now a hard dependency and MUST be executed locally before pushing any changes to CI configurations. Pushing unverified SHAs that cause production-only failures is strictly forbidden.
* **Environment Parity Lesson**: The agent MUST ensure that any maintenance tools requiring remote access or authentication (e.g., `gh api`, `trivy`) have their required secrets (like `GITHUB_TOKEN`) explicitly provided in the `.github/workflows/ci.yml` environment. Disparity between local (authenticated) and production (unauthenticated) environments is a failure of the CI validation loop.
