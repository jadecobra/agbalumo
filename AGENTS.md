...
* **General TDD**: ALWAYS write a failing test (RED) before writing implementation code. Run `go run cmd/verify/main.go precommit` before `go test ./...`. Read raw tracebacks to self-correct natively.
