...
func TestAdminHandler_HandleDashboard_HappyPath(t *testing.T) {
	...
	// 2. Listings
	_ = env.App.DB.Save(ctx, domain.Listing{ID: "l1", Title: "Business A", Type: domain.Business, IsActive: true, OwnerOrigin: "Nigeria", Address: "123 Lagos St"})
	_ = env.App.DB.Save(ctx, domain.Listing{ID: "l2", Title: "Job B", Type: domain.Job, IsActive: true, OwnerOrigin: "Ghana"})

	// 3. Claim Requests
	_ = env.App.DB.SaveClaimRequest(ctx, domain.ClaimRequest{ID: "c1", UserID: "u2", ListingID: "l1", Status: domain.ClaimStatusPending})

	// 4. Feedback
	_ = env.App.DB.SaveFeedback(ctx, domain.Feedback{ID: "f1", Type: domain.FeedbackTypeIssue, Content: "Help!"})

	// 5. Categories
	_ = env.App.DB.SaveCategory(ctx, domain.CategoryData{ID: "music", Name: "Music"})
...
func TestAdminHandler_HandleDashboard_GrowthMetrics(t *testing.T) {
	...
	now := time.Now()
	_ = env.App.DB.SaveUser(ctx, domain.User{ID: "u1", CreatedAt: now})
	_ = env.App.DB.Save(ctx, domain.Listing{ID: "l1", CreatedAt: now})
