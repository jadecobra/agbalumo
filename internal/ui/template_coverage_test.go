...
var baseData = module.BaseViewData{
	...
	User:             &mockUser,
	CSRF:             "test-csrf",
	Categories:       []domain.CategoryData{{Name: "Food"}, {Name: "Job"}, {Name: "Event"}, {Name: "Business"}},
...
var TemplateMockRegistry = map[string]interface{}{
	...
	"sandbox.html": common.SandboxViewModel{
		BaseViewData:          baseData,
		MockListingNormal:     mockListing,
		MockListingFeatured:   mockListing,
		MockListingJob:        mockListing,
		MockListingEvent:      mockListing,
		MockListingBusiness:   mockListing,
		MockListingZeroRating: mockListing,
		MockListingNotOwned:   mockListing,
		MockUser:              &mockUser,
		MockSavedIDs:          map[string]bool{},
		MockPagination1:       domain.Pagination{Page: 1, TotalPages: 1},
		MockPagination2:       domain.Pagination{Page: 1, TotalPages: 1},
		...
		MockProfileData: map[string]interface{}{
			"SavedIDs":         map[string]bool{},
			"Listings":         []domain.Listing{mockListing},
			"SavedListings":    []domain.Listing{mockListing},
			"User":             &mockUser,
			"Env":              "test",
			"CSRF":             "test-csrf",
			"GoogleMapsApiKey": "test-api-key",
		},
		MockCreateData: baseData,
		MockEditData: map[string]interface{}{
			"Listing":          mockListing,
			"Categories":       []domain.CategoryData{{Name: "Food"}, {Name: "Job"}, {Name: "Event"}, {Name: "Business"}},
			"User":             &mockUser,
			"Env":              "test",
		...
		MockFeedbackData: map[string]interface{}{
			"Path": "/sandbox",
			"User": &mockUser,
			"Env":  "test",
			"CSRF": "test-csrf",
		},
		MockCategories:    []domain.CategoryData{{Name: "Food"}},
		MockCounts:        map[string]int{"Food": 1},
		MockAdminListings: []domain.Listing{mockListing},
		MockAdminFeedback: domain.Feedback{},
		MockAdminMetrics: map[string]interface{}{
			"ListingGrowth":   []domain.DailyMetric{{Date: "2024-01-01", Count: 1}},
			"UserGrowth":      []domain.DailyMetric{{Date: "2024-01-01", Count: 1}},
			"UserCount":       128,
			"ListingCount":    49,
			"AdaDiscoveryAvg": 4.2,
			...
			"Pagination":      domain.Pagination{Page: 1, TotalPages: 1},
			"Categories":      []domain.CategoryData{{Name: "Food"}, {Name: "Job"}, {Name: "Event"}, {Name: "Business"}},
			"Counts":          map[string]int{"Food": 1, "Job": 1, "Event": 1, "Business": 1},
			"User":            &mockUser,
			"Env":             "test",
			"CSRF":            "test-csrf",
		},
		MockAdminUsers:   []domain.User{mockUser},
		MockToastID:      "toast-sandbox-error",
		MockToastTitle:   "Error",
		MockToastMessage: "Message",
	},
