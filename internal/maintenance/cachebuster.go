...
func VerifyCacheBuster(rootDir string) error {
	if err := VerifyCSSCacheBuster(rootDir); err != nil {
		return err
	}
	return VerifyJSCacheBuster(rootDir)
}

// VerifyCSSCacheBuster verifies only output.css hash consistency in head_meta.html.
func VerifyCSSCacheBuster(rootDir string) error {
...
// VerifyJSCacheBuster scans all HTML templates for unversioned local custom script tags.
func VerifyJSCacheBuster(rootDir string) error {
	...
	err := filepath.Walk(templatesDir, func(path string, info os.FileInfo, walkErr error) error {
		...
		return checkSingleTemplateScripts(rootDir, path, reScript)
	})
...
func checkSingleTemplateScripts(rootDir, path string, reScript *regexp.Regexp) error {
