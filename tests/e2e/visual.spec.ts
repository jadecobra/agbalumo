...
test.describe('Listing Modal Visual Regression', () => {
  test('capture and diff the listing detail modal', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByTestId('ag-listing-card').filter({ hasText: 'Aria Nigerian Suya Kitchen' }).first()).toBeVisible();
    await page.getByTestId('ag-listing-card').filter({ hasText: 'Aria Nigerian Suya Kitchen' }).first().click();
