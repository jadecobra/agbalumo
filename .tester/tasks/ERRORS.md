...
- agent created script to update coverage threshold programmatically, then deleted the program 
...
- agent used curl when stopped from using browser_subagent and using github cli failed with permissions error
...
- agent skills should include specific scripts/agent-exec.sh command for the task so agent doesn't have to read help on every run
- agent skills scripts not implemented yet - start with make it red, security
- critique make it better skill
...
- do we need a janitor persona that goes through and cleans out unused files? deleter
- review agent and skills for lego brick and fast iteration cycles
- Taskfile.yml is now 300 lines
