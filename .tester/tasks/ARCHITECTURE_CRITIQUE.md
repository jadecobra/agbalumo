...
- [ ] **Modularization: Phase 3 - Standardized Assembly (Software Factory)**
  ...
  - [x] Task 3.4: In `internal/module/admin/admin.go`, create an `AdminDependencies` struct. Update `NewAdminHandler` to accept this struct instead of multiple arguments. Fix `admin` tests.
  ...
  - [x] Task 3.8: Clean up `cmd/server.go`. It should now construct dependency structs, initialize the modules, and call `RegisterRoutes` on each. Run `./scripts/pre-commit.sh` to validate.
